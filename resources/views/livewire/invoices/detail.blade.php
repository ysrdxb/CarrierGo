...
<style>
        ...
        <div class="side-app ">
            <div class="main-container container">
                <div class="page-header">
                    <button class="btn btn-secondary bg-secondary-gradient mt-3 d-print-none" data-bs-toggle="modal" data-bs-target="#addServiceModal" wire:click="addItem()">+ Add service</button>
                </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="table-responsive push">
                                    <table class="table table-bordered table-hover mb-0 text-nowrap">
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center">1</td>
                                                <td class="text-center">{{ session()->has('currency') ? session('currency') : '€' }} {{ number_format($invoice->reference->price, 2) }}</td>
                                                <td class="text-center">{{ session()->has('currency') ? session('currency') : '€' }} {{ number_format($invoice->reference->price, 2) }}</td>
                                            </tr>
                                            @php $i =  2; @endphp
                                            @else
                                            ...
                                            @foreach($invoice->items as $item)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <p class="font-w600 mb-1">{{ $item->name }}
                                                            <a style="margin-right:5px" href="#;" class="d-print-none pull-right" data-bs-toggle="modal" data-bs-target="#addServiceModal" wire:click="editItem({{ $item->id }})"><i class="fe fe-edit"></i></a>
                                                            <a style="margin-right:5px" href="javascript:void(0)" onclick="confirm('Are you sure you want to remove this item?') || event.stopImmediatePropagation()" wire:click="removeItem({{ $item->id }})" class="d-print-none pull-right ml-2"><i class="fa fa-trash"></i></a>
                                                        </p>
                                                        <p class="font-w600 mb-1">{{ $item->description }}</p>
                                                    </td>
                                                    <td class="text-center">{{ $item->quantity }}</td>
                                                    <td class="text-center">{{ session()->has('currency') ? session('currency') : '€' }} {{ number_format($item->price, 2) }}</td>
                                                    <td class="text-center">{{ session()->has('currency') ? session('currency') : '€' }} {{ number_format($item->price * $item->quantity, 2) }}</td>
                                                </tr>
                                            @endforeach
                                            @if($invoice->reference->extra_fees_eur > 0)
                                                <tr>
                                                    ...
                                                    <td class="text-center">1</td>
                                                    <td class="text-center">{{ session()->has('currency') ? session('currency') : '€' }} {{ number_format($invoice->reference->extra_fees_eur, 2) }}</td>
                                                    <td class="text-center">{{ session()->has('currency') ? session('currency') : '€' }} {{ number_format($invoice->reference->extra_fees_eur, 2) }}</td>
                                                </tr>
                                                @php
                                            ...
                                            @if($invoice->reference->additionalFees)
                                                @foreach($invoice->reference->additionalFees as $row)
                                                    <tr>
                                                        ...
                                                        <td class="text-center">1</td>
                                                        <td class="text-center">{{ session()->has('currency') ? session('currency') : '€' }} {{ number_format($row->amount, 2) }}</td>
                                                        <td class="text-center">{{ session()->has('currency') ? session('currency') : '€' }} {{ number_format($row->amount, 2) }}</td>
                                                    </tr>
                                                    @php
                                                        $subtotal += $row->amount;
                                    ...
                                    <br>
                                    <div>Amount is due immediately!</div>
                                </div>
                            </div>
    ...
    @include('components.add-invoice-items-modal')
</div>
